...
class BinaryTree{
    public:
        friend class OpTree;
        friend ostream& operator<<(ostream& os,const Forest<T>& forest);
...
class Forest{
    public:
        Forest(){}
        Forest(const vector<BinaryTree<T>>& subTrees)
            :subTrees(subTrees) {}
        
        //Forest BinTree 를 받으면 Forest로 분리해서 저장하는거.
        
        friend ostream& operator<<(ostream& os,const Forest<T>& forest){
            for(int i=0;i<forest.subTrees.size();i++){
                // forest.subTrees[i].InOrder();
                forest.subTrees[i].PreOrder();
            }
            return os;
        }
        void InsertSubtree(const BinaryTree<T>& subtree){
            subTrees.push_back(subtree);
        }
    private:
        //BinTree로 반환(operator? 그 전에 일단 private함수로 만들어놓고 여기저기 쓰면 될듯)
        BinaryTree<T>& forestToTree() const{
            BinaryTree<T> ret;
            TreeNode<T>* root;
        ...
        BinaryTree<T>& toForestTree(const BinaryTree& tree) const{
            
        }
        vector<BinaryTree<T>> subTrees;
};
